...
local SetupPages = {
   ...
   },
   {
      title = "Rates ",
      text = {
         { t = "Super rates", x = 1,  y = 10 },
         { t = "RC",          x = 70, y = 10 }
      },
      fields = {
         -- Super Rate
         { t = "Roll",  x = 1,  y = 24, sp = 30, i=3 },
         { t = "Pit.", x = 1,  y = 34, sp = 30, i=4 },

         -- Roll + Pitch
         { t = "Rt",  x = 50,  y = 24, sp = 20, i=1 },
         { t = "Ex.",  x = 90, y = 24, sp = 20, i=2 },

         -- Yaw
         { t = "Yaw",   x = 1,  y = 54, sp = 30, i=5 },
         { t = "Rt",  x = 50,  y = 54, sp = 20, i=12 },
         { t = "Ex.",  x = 90, y = 54, sp = 20, i=11 },
      },
      read  = MSP_RC_TUNING,
   ...
   },
   {
      ...
      fields = {
         -- VTX
         { t = "Band",    x = 1,  y = 14, sp = 50, i=2, min=1, max=5, table = { "A", "B", "E", "F", "R" } },
         { t = "Channel", x = 1,  y = 24, sp = 50, i=3, min=1, max=8 },
         { t = "Power",   x = 1,  y = 34, sp = 50, i=4, min=1 },
         { t = "Pit",     x = 1,  y = 44, sp = 50, i=5, min=0, max=1, table = { [0]="OFF", "ON" } },
         { t = "(Dev)",   x = 1, y = 54, sp = 50, i=1, ro=true, table = {[3]="SmartAudio",[4]="Tramp"} },
      },
      read  = MSP_VTX_CONFIG,
...
local function drawScreen(page,page_locked)
   ...
   lcd.drawText(1,1,"Betaflight setup: "..screen_title,INVERS)
...
local function run(event)
   ...
   elseif gState == MENU_DISP then
      if event == EVT_EXIT_BREAK then
         gState = PAGE_DISPLAY
      elseif event == EVT_PLUS_BREAK or event == EVT_ROT_LEFT or event == EVT_ROT_DOWN then
         incMenu(-1)
      elseif event == EVT_MINUS_BREAK or event == EVT_ROT_RIGHT or event == EVT_ROT_UP then
         incMenu(1)
   ...
   elseif gState == EDITING then
      if (event == EVT_EXIT_BREAK) or (event == EVT_ENTER_BREAK) then
         gState = PAGE_DISPLAY
      elseif event == EVT_PLUS_FIRST or event == EVT_PLUS_REPT or event == EVT_ROT_RIGHT or event == EVT_ROT_UP then
         incValue(1)
      elseif event == EVT_MINUS_FIRST or event == EVT_MINUS_REPT or event == EVT_ROT_LEFT or event == EVT_ROT_DOWN then
         incValue(-1)
      end
   end
   ...
   if getValue("RSSI") == 0 then
      -- No!
      lcd.drawText(68,56,"No telemetry",SMLSIZE + BLINK)
      invalidatePages()
   end
   ...
   elseif gState == PAGE_SAVING then
	  ...
	  local y = 12
	  local w = 100
      lcd.drawFilledRectangle(x,y,w,30,ERASE)
      lcd.drawRectangle(x,y,w,30,SOLID)
      lcd.drawText(x+4,y+3,"Saving...",DBLSIZE + BLINK)
   end
